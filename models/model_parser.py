...
if len(sys.argv) > 4:
    IPU_CAPACITY_MB = int(sys.argv[4])
    IPU_CAPACITY_KB = IPU_CAPACITY_MB * 1024
    IPU_CAPACITY_B = IPU_CAPACITY_KB * 1024
    IPU_CAPACITY_ELEM = IPU_CAPACITY_B // 2
...
while not partition_ready:
    ...
    for i in range(len(json_dump_list)):
        # if length of op_inputs is 2
        sorted_dims = sorted(json_dump_list[i][1])
        if json_dump_list[i][0] == "BatchMatMul" and sorted_dims[-1]*sorted_dims[-2]*sorted_dims[-3]*sorted_dims[-4] > IPU_CAPACITY_ELEM:
            ...
            op_type_name, dim_lengths, variables, ignore_variables, op_type_id, op_id, op_inputs = copy.deepcopy(json_dump_list[i])
            ...
            partition_ready = False
            break
        elif len(json_dump_list[i][6]) == 2 and sorted_dims[-1]*sorted_dims[-2] > IPU_CAPACITY_ELEM:
            # print (f"Splitting Dot op {json_dump_list[i][6]}")
            max_dim_idx = json_dump_list[i][1].index(max(json_dump_list[i][1]))
            json_dump_list[i][1][max_dim_idx] //= 2
            op_type_name, dim_lengths, variables, ignore_variables, op_type_id, op_id, op_inputs = copy.deepcopy(json_dump_list[i])
